...
// Run:
// gulp scripts.
// Uglifies and concat all THEME JS files into one
...
gulp.task( 'adminscripts', function() {
  ...
  gulp
    .src( admin_scripts, { allowEmpty: true })
    .pipe(babel(
      {
        presets: ['@babel/preset-env']
      }
...
gulp.task( 'customizer', function() {
  ...
  gulp
    .src( customizer_scripts, { allowEmpty: true })
    .pipe(babel(
      {
        presets: ['@babel/preset-env']
      }
...
gulp.task('compile', gulp.series('styles', 'scripts', 'adminscripts', 'customizer', 'dist'));
