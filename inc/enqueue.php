...
if ( ! function_exists( 'evstrap_scripts' ) ) {
	...
	function evstrap_scripts() {
		...
		if ($minify_assets) {
			...
			wp_enqueue_style( 'evstrap-styles', get_template_directory_uri() . '/css/theme.css', array(), $css_version );
	
			wp_enqueue_script( 'jquery' );
	
			$js_version = $theme_version . '.' . filemtime( get_template_directory() . '/js/theme.js' );
			wp_enqueue_script( 'evstrap-scripts', get_template_directory_uri() . '/js/theme.js', array(), $js_version, true );
		}
